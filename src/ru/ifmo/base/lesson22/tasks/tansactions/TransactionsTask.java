...
public class TransactionsTask {
    public static void main(String[] args) {
        ...
        Stream<Transaction> transactionStream2 = Stream.of(transaction1, transaction2, transaction3,
                transaction4, transaction5, transaction6, transaction7);
        ...
        System.out.println("---Группировка по номер аккаунта,сумма транзакций---");
        Map<String, Long> byAcc = transactionStream
                .collect(Collectors.groupingBy(t -> t.getAccount().getNumber(),
                        Collectors.summingLong(Transaction::getSum)));
        byAcc.entrySet().forEach(System.out::println);

        System.out.println("---найти сумму транзакций по каждому аккаунту---");
        Long byAccSum = transactionStream2.mapToLong(Transaction::getSum).sum();
        System.out.println("сумма транзакций = " + byAccSum);
//
