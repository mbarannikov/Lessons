...
public class MapTask {
    ...
    public static Map<String, Customer> fromToCustomer(Map<String, Customer> customerMap, int from, int to) {
        ...
        HashMap<String, Customer> newCustomerMap = new HashMap<>();
    ...
    public static Map<Integer, List<String>> topTen(List<String> stringList) {
        //  3. написать метод, который выводит в консоль топ 10 самых частых слов
        HashMap<String, Integer> stringMap = new HashMap<>();
        TreeMap<Integer, List<String>> topMap = new TreeMap<>();
        Integer value;
        for (String entry: stringList
        ) {
            char[] chr = entry.toCharArray();
            value = stringMap.get(entry);
            if(value == null) stringMap.put(entry,1);
            else stringMap.replace(entry, ++value);
        }
...
//        }
        for (Map.Entry<String,Integer> entry: stringMap.entrySet()
            ...
            if(topMap.containsKey(key)) {
                List<String> listString = new ArrayList<String>(topMap.get(key));
                listString.add(strValue);
                topMap.replace(key, listString);
            }
            else topMap.put(key,Arrays.asList(strValue));

        }
//        System.out.println("topMap" + topMap);
        return topMap.descendingMap();
    }
